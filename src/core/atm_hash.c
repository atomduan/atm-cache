...
{
    atm_hash_t *hash 
        = (atm_hash_t *) atm_malloc(sizeof(atm_hash_t));
    hash->table 
        = (atm_hash_buck_t *) atm_malloc(sizeof(atm_hash_buck_t));
...
{
    atm_hash_entry_t *result 
        = atm_malloc(sizeof(atm_hash_entry_t));
    result->key = key; 
    result->value = value; 
    result->next = NULL;
    return result;
...
atm_bool_t atm_hash_set(atm_hash_t *hash, 
        atm_string_t *key, void *value)
{
    ...
    atm_hash_entry_t *entry 
        = atm_hash_get_entry(hash, key);
